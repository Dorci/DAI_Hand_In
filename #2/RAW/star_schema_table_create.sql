...
INSERT INTO star_schema.d_product(dimension_product_id, name)(SELECT dimension_product_id, name FROM StagingDatabase.staging.stage_dim_product);
--This statement inserts attribute vales into dimension d_customer table
INSERT INTO star_schema.d_customer(dimension_customer_id, title, first_name, middle_name, last_name)(SELECT dimension_customer_id, title, first_name, middle_name, last_name FROM StagingDatabase.staging.stage_dim_customer);
...
INSERT INTO star_schema.f_sales(sales_id, customer_id, product_id, date_id, quantity, line_total) (SELECT sales_id, customer_id, product_id, date_id, quantity, line_total FROM StagingDatabase.staging.stage_f_sales);

-- This statement returns Top seller products
SELECT FS.product_id, name, COUNT(*) as COUNT FROM star_schema.f_sales AS FS JOIN star_schema.d_product ON FS.product_id = d_product.dimension_product_id GROUP BY FS.product_id, name ORDER BY COUNT DESC;
